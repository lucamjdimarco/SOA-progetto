...
bool hasNullTerminator(const char *str) {
    // Itera attraverso la stringa fino a quando non trova il terminatore nullo '\0'
    while (*str != '\0') {
        str++;  // Passa al prossimo carattere
    }

    // Se arriva qui, significa che ha trovato il terminatore nullo
    return true;
}
